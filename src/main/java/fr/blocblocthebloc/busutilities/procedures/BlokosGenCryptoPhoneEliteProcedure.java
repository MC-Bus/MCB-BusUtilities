...
public class BlokosGenCryptoPhoneEliteProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (entity.isShiftKeyDown()) {
			...
			if (BusutilitiesModVariables.MapVariables.get(world).CryptoPhoneGenState == false) {
				...
				if (entity instanceof Player _player && !_player.level.isClientSide())
					_player.displayClientMessage(Component.literal("[CryptoPhone] : 2 Blokos \u00E0 \u00E9t\u00E9 ajout\u00E9 \u00E0 votre inventaire"), true);
			} else {
				if (entity instanceof Player _player)
					_player.getCooldowns().addCooldown(itemstack.getItem(), 200);
				{
					double _setval = (entity.getCapability(BusutilitiesModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new BusutilitiesModVariables.PlayerVariables())).Blokos + 2;
